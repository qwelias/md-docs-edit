...
const rebuild = () => {
	...
	return Promise.all( [ Doc.find().exec(), Group.find().exec() ] ).then( ( [ docs, groups ] ) => {
		...
		const mkdocs = Object.assign( {}, config.doc.def, {
			pages
		} );

		mkdocs.site_dir = path.resolve( config.root, config.doc.path, mkdocs.site_dir );
		mkdocs.docs_dir = path.resolve( config.root, config.doc.path, mkdocs.docs_dir );

		Log( 'CONFIG', YAML.stringify( mkdocs ) );

		rimraf.sync( path.resolve( mkdocs.docs_dir, '*.md' ) );
		mkdirp.sync( path.resolve( mkdocs.docs_dir ) );

		return Promise.all(
			[ new Promise( ( resolve, reject ) => {
				fs.writeFile(
					path.resolve( config.root, config.doc.path, 'mkdocs.yml' ),
					YAML.stringify( mkdocs ),
				...
				docs.map( d => {
					return new Promise( ( resolve, reject ) => {
						fs.writeFile(
							path.resolve( mkdocs.docs_dir, d.fileName ),
							d.body,
							e => e ? reject( e ) : resolve()
						)
					} )
		...
		return new Promise( ( resolve, reject ) => {
			ncp(
				path.resolve( config.root, config.doc.path, 'override' ),
				path.resolve( config.root, config.doc.path, config.doc.def.site_dir ),
				e => e ? reject( e ) : resolve()
			);
		} );
	} ).catch( e => {
